...
option_list = list(
  ...
  make_option(
    ...
    default = "any",
    type = 'string',
    help = 'A string specifying how p-values are to be combined across pairwise comparisons for a given group/cluster. Setting pval.type="all" requires a gene to be DE between each cluster and every other cluster (rather than any other cluster, as is the default with pval.type="any").'
  ),
  make_option(
    c("-s", "--subset_row"),
    action = "store",
  ...
  make_option(
    ...
    default = NA,
    type = 'character',
    help = 'Path to the rds object containing a list of DataFrames, each of which contains a sorted marker gene list for the corresponding group.'
  )
)

opt = wsc_parse_args(option_list, mandatory = c("input_sce_object", "output_markers"))

#read SCE object
if(!file.exists(opt$input_sce_object)) stop("Input file does not exist.")
sce <- readRDS(opt$input_sce_object)

#if only counts are present, binom test should be used --> The function already throws an error!
#if(!opt$assay_type %in% names(assays(sce)) & opt$type != "binom"){
#    stop("If SCE only contains raw counts, it can only be used with test.type='binom'. ")
#}

#Find Marker Genes
markers <- findMarkers(sce, groups=opt$groups, test.type=opt$test_type, subset.row = opt$subset_row, assay.type = opt$assay_type, pval.type=opt$pvalue_type, get.spikes=opt$get_spikes)
